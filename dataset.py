...
class AudiosetDataset(Dataset):
    def __init__(self, dataset_json_file, audio_conf, label_csv=None, use_fbank=False, fbank_dir=None, mask_type='none', mask_ratio=0.5, roll_mag_aug=False, load_video=False, mode='train'):
        """
    ...
    def _wav2fbank(self, filename, filename2=None):
        ...
        # ===== 時間遮罩（Time Masking） =====
        if self.mask_type == 'time' and self.mask_ratio > 0:
            L = int(T * self.mask_ratio)
            if 0 < L < T:
                t0 = random.randint(0, T - L)
                fbank[t0:t0+L, :] = 0.0
        
        # ===== 頻率遮罩（Freq Masking） =====
        elif self.mask_type == 'freq' and self.mask_ratio > 0:
            L = int(F * self.mask_ratio)
            if 0 < L < F:
                f0 = random.randint(0, F - L)
                # 沿著頻率軸遮一整條（所有時間都被那幾個頻帶設成 0）
                fbank[:, f0:f0+L] = 0.0

        # ===== 時間 + 頻率 timefreq 遮罩（timefreq Block Masking） =====
        elif self.mask_type == 'timefreq':
            time_ratio = 0.25      
            freq_ratio = 0.25      
            num_blocks = 4

            block_T = max(1, int(T * time_ratio))
            block_F = max(1, int(F * freq_ratio))

            for _ in range(num_blocks):
                if block_T >= T or block_F >= F:
                    break
                t0 = random.randint(0, T - block_T)
