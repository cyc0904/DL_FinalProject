...
def get_args_parser():
    ...
    parser.add_argument(
        '--mask_type',
        default='none',
        choices=['none', 'time', 'freq', 'timefreq', 'patch'],
        help='Masking strategy for input features. "none" = no masking (baseline).'
    )
    ...
    parser.add_argument(
        ...
        default=0.5,
        type=float,
        help='Ratio of time/freq/patch to mask when mask_type != "none".'
    )
...
def main(args):
    ...
    if not args.audio_exp:
        ...
        target_length = {'audioset':128, 'esc50':512, 'speechcommands':1024}
        multilabel_dataset = {'audioset': True, 'esc50': False, 'k400': False, 'speechcommands': False}
        ...
        audio_conf_val = {'num_mel_bins': 128, 
                      ...
                      'multilabel':multilabel_dataset[args.dataset],
                      'noise':False}                      
        dataset_train = AudiosetDataset(args.data_train, label_csv=args.label_csv, audio_conf=audio_conf_train, use_fbank=args.use_fbank, fbank_dir=args.fbank_dir, mask_type=args.mask_type, mask_ratio=args.mask_ratio)
        dataset_val = AudiosetDataset(args.data_eval, label_csv=args.label_csv, audio_conf=audio_conf_val, use_fbank=args.use_fbank, fbank_dir=args.fbank_dir, mask_type=args.mask_type, mask_ratio=args.mask_ratio)
    ...
    if args.audio_exp:
        ...
        in_chans=1
        if getattr(args, 'use_custom_patch', False):
    ...
    if args.finetune and not args.eval:
        ...
        if 0: #not args.audio_exp:	
            ...
            if getattr(args, 'use_custom_patch', False):
                ...
                if getattr(args, 'source_custom_patch', False):
                    # don't do this	
